  ...
  GetTeacher = (req, res, next) => {
    ...
    return this.database.query(query, (err, result, affected) => {
      if (err) {
        ...
        const Query = `SELECT AT.EMAIL AS EMAIL, T.ID AS ID, T.DESIGNATION AS DESIGNATION, T.AOI AS AREAOFINTREST, S.ID AS EID, S.GENDER AS GENDER, S.FIRST_NAME AS FIRSTNAME,S.LAST_NAME AS LASTNAME, S.NATIONALITY AS NATIONALITY, S.CNIC AS CNIC, S.RELIGON AS RELIGON, S.POSTAL_CODE AS POSTCODE, S.CITY AS CITY, S.STREETNO AS STREETNO, S.HOUSENO AS HOUSENO, S.STATE AS STATE, S.COUNTRY AS COUNTRY, S.DOB AS DOB, S.JOINING_DATE AS JOININGDATE, S.PHONENUMBER AS PHONENUMBER, P.NAME AS PROGRAMNAME, P.TOTAL_FEES AS TOTALFEES, P.DURATION AS DURATION, P.TOTAL_CREDIT_HOURS AS CREDITHOURS, P.ID AS PROGRAMID FROM TEACHERS T JOIN EMPLOYEMENT S ON S.ID=T.EMPLOYEE_ID JOIN PROGRAM P ON P.ID=S.PROGRAM_ID JOIN AUTHENTICATION AT ON S.ID=AT.USER_ID   ${
          id !== null ? "WHERE T.ID =" + id + " AND AT.ROLL_ID=3" : "WHERE AT.ROLL_ID=3"
