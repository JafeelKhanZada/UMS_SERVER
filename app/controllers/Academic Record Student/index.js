...
class AcademicRecordStudent extends DB {
  ...
  InsertData = (req, res, next) => {
    ...
    const Query = `INSERT INTO ACADEMIC_RECORD_STUDENT (STUDENT_ID,YOGRADUATION,TOTAL_MARKS,OBTIAN_MARKS,BOARD_NAME,DEGREE) VALUES ('${sID}','${yearGradution}','${totalMarks}','${obtainMarks}','${boardName}','${degree}')`;
  ...
  GetData = (req, res, next) => {
    ...
    const query = `SELECT count(*) as total FROM ACADEMIC_RECORD_STUDENT ${
      id !== null ? "WHERE ID =" + id : ""
    }`;
    return this.database.query(query, (err, result, affected) => {
      if (err) {
        res.json({
          error: true,
          message: "Error Occurs At Get ACADEMIC_RECORD_STUDENT Data!",
          data: err
        });
        next();
      } else {
        ...
        const Query = `SELECT * FROM ACADEMIC_RECORD_STUDENT S ${
  ...
  DeleteData = (req, res, next) => {
    ...
    const query = `DELETE FROM ACADEMIC_RECORD_STUDENT WHERE ID IN(${string})`;
    return this.database.query(query, (err, result, affected) => {
      if (err) {
        res.json({
          error: true,
          message: "Error Occurs At Delete ACADEMIC_RECORD_STUDENT!",
          data: err
        });
        next();
      } else {
  ...
  UpdateData = (req, res, next) => {
    const { id } = req.params;
    const Query = `UPDATE ACADEMIC_RECORD_STUDENT SET ? WHERE ID=${id}`;
    return this.database.query(
      ...
      (err, result, affected) => {
        if (err) {
          res.json({
            error: true,
            message: "Error Occurs At Update ACADEMIC_RECORD_STUDENT!",
            data: err
          });
          next();
        } else {
